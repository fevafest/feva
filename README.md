...
5. **Note:** Render's free tier has an ephemeral filesystem — anything saved to `backend/uploads` (event posters, blog covers, avatars uploaded through the app) is wiped on every redeploy/restart. Fine for testing; for real production use, say the word and I'll switch file uploads to Cloudinary or S3 (needs an account/API key from you either way).
...
**Seeding admins on the real database:** once you share the MongoDB URI, run (from your machine, or I can run it for you):
```bash
cd backend
MONGO_URI="<your atlas uri>" node src/utils/seed.js
```
This wipes and reseeds **everything** (users, events, blog, organizer) — fine for a first-time setup, but don't run it again once real customers/orders exist. If you just want the superadmin account without touching real event data, tell me and I'll write a smaller "just create this one admin" script instead.
...
- ⏳ **MongoDB URI** — send it over and I'll seed the superadmin/admin accounts against it directly.
...
- **Optional — persistent file storage:** Render's free tier wipes uploaded images on redeploy. Say the word if you want event posters/blog covers/avatars moved to Cloudinary or S3 instead — needs an account/API key from you.
